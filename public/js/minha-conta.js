...
async function loadOrderHistory(user, container) {
    ...
    try {
        ...
        querySnapshot.forEach(doc => {
            ...
            // Se o pedido estiver pendente e tivermos um link do Mercado Pago guardado, mostramos o botão
            const botaoPagar = (order.status === 'pending' && order.paymentUrl) 
                ? `<a href="${order.paymentUrl}" target="_blank" style="display: inline-block; margin-top: 10px; padding: 8px 16px; background-color: #28a745; color: white; text-decoration: none; border-radius: 5px; font-weight: bold;">💸 Pagar Agora</a>` 
                : '';

...
const loadProducts = () => {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        snapshot.forEach(docSnap => {
            ...
            const nomeComBadge = isPromo ? `${product.name} <br><span style="background-color: #e74c3c; color: white; padding: 3px 8px; border-radius: 12px; font-size: 0.75rem; font-weight: bold; display: inline-block; margin-top: 5px;">🔥 Promoção Ativa</span>` : product.name;
            const precoFormatado = isPromo ? `<span style="text-decoration: line-through; color: #999; font-size: 0.85em;">${BRL(product.price)}</span> <br> <strong>${BRL(product.promotionalPrice)}</strong>` : BRL(product.price);
            ...
            tr.innerHTML = `
                ...
                <td class="actions-cell">
                    <button class="action-btn-icon edit" data-id="${docSnap.id}"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg></button>
                    <button class="action-btn-icon delete" data-id="${docSnap.id}"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg></button>
                </td>
            `;
    ...
    onSnapshot(q, (snapshot) => {
        ...
        snapshot.forEach(docSnap => {
            ...
            tr.innerHTML = `
                ...
                <td><strong>${customer.name || 'Sem Nome'}</strong><br><small>📱 ${customer.phone || 'Sem telefone'}</small></td>
                <td>${orderDate}</td>
                ...
                <td class="order-actions">${(order.status === 'pending' || order.status === 'paid') ? `<button class="action-btn ship" data-id="${orderId}">Enviar</button><button class="action-btn cancel" data-id="${orderId}">Cancelar</button>` : ''}</td>
            `;
            ...
            detailsTr.innerHTML = `
                <td colspan="6">
                    <div class="order-details-content" style="display: flex; gap: 2rem; text-align: left; padding: 1rem; background-color: #f9f9f9; border-radius: 8px;">
                        ...
                        <div style="margin-top: 1rem; padding-top: 1rem; border-top: 1px solid #ddd;"><strong>Entrega:</strong><br><span style="color: var(--cor-secundaria);">${formattedAddress}</span></div></div>
...
ordersTableBody.addEventListener('click', async (e) => {
    ...
    if (actionButton) {
        ...
        if (actionButton.classList.contains('ship')) {
            if (await showConfirmation('Marcar como Enviado?', 'O estado será "Enviado".')) {
            ...
            if (await showConfirmation('Cancelar este Pedido?', 'Ação irreversível.')) {
...
productsTableBody.addEventListener('click', async (e) => {
    ...
    if (target.classList.contains('delete')) {
        if (await showConfirmation('Tem a certeza?', 'Produto apagado permanentemente.')) {
            try {
            ...
            } catch (error) { showToast('Falha ao apagar.', 'error'); }
        }
    }
    ...
    try {
        ...
        if (files.length > 0) {
            ...
            if (isEditing && existingImageUrls.length > 0) {
                 for (const url of existingImageUrls) await deleteObject(ref(storage, url)).catch(() => {});
            }
        }
...
if (themeForm) {
    themeForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (finalLogoUrl) themeData.logoUrl = finalLogoUrl;
        ...
        } catch (error) { showToast('Erro ao atualizar tema.', 'error'); } 
        finally {
